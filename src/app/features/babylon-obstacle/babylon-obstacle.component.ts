...
export class BabylonObstacleComponent implements OnInit, OnDestroy {
  ...
  private localObstacle: Obstacle;
  ...
  private subscribeToObstacles() {
    ...
    this.obstacleGenerationService.obstacleUpdates$
      ...
      .subscribe(updatedObstacle => {
        if (updatedObstacle) {
          // Clone obstacle data to localObstacle to avoid affecting external data
          this.localObstacle = updatedObstacle;
          this.createOrUpdate3DObstacle(this.localObstacle);
        }
      });
  }
